...
export function BacklogSidebar({ backlogTasks, onAddTask }: BacklogSidebarProps) {
  ...
  return (
    <aside
      className={`flex w-80 shrink-0 flex-col overflow-hidden border-r border-white/5 bg-zinc-900/50 transition-all duration-300 ${
        isCollapsed ? "h-auto self-start" : "h-full"
      }`}
    >
      <div className="border-b border-white/5 px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          ...
          >
            {isCollapsed ? <ChevronUp className="h-3.5 w-3.5" aria-hidden="true" /> : <ChevronDown className="h-3.5 w-3.5" aria-hidden="true" />}
          </Button>
        </div>
      </div>

      <div className={`flex h-full min-h-0 flex-col gap-4 p-3 transition-all duration-300 ${isCollapsed ? "pointer-events-none max-h-0 overflow-hidden opacity-0 p-0" : "opacity-100"}`}>
